...
This agent dispatches on Saturdays at 11:00 and provides me with a list of new restaurants to try.`]},{label:"Pipeline",body:"Each agent run is a multi-phase pipeline managed by Python - no single model call holds the full context. A planner LLM generates a set of search queries tailored to the agent's goal and sources. Those queries fire in parallel across all configured search providers - DuckDuckGo, Brave, Jina, and Ollama Cloud. A validator then reads the collected results and decides whether the research buffer is sufficient; if not, it generates retry queries and runs another pass. Once coverage passes, a synthesizer formats the final output using the agent's output template. Agents can also run multi-track: independent sub-pipelines execute in parallel, each with its own sources and goal, and their results get aggregated into a single response, a digest."},{label:"Search fan-out",body:"Every search query fans out in parallel across up to four providers - DuckDuckGo, Brave Search, Jina Search, and Ollama Cloud. Results are merged and deduplicated. DuckDuckGo is always on as a baseline; the others activate when you provide an API key. All four offer free tiers."},{label:"Web fetching",body:"For agents that need full article content rather than search snippets, the pipeline ranks result URLs by relevance, fetches the top pages, and runs a per-page summarizer before synthesis. Raw HTTP with tag-stripping is the default; if a page returns thin content (under 3000 chars - usually a JavaScript shell), Jina Reader is called as a fallback to get the rendered version."},{label:"Direct feed mode",body:"Agents can skip the search planner entirely and pull from a list of known URLs - RSS feeds, API endpoints, anything on a predictable schedule. {today} is substituted with the current date, so date-parameterized endpoints work without configuration changes."},{label:"Multi-track pipelines",body:"A single agent can run multiple independent sub-pipelines in parallel, each with its own goal, sources, freshness filter, and query count. Results are aggregated into one synthesizer call. Useful for agents that cover distinct angles - for example, research papers and industry news running as separate tracks under one agent."},{label:"LLM providers",body:"The inference layer is provider-agnostic. Ollama, OpenAI, Anthropic, and Fireworks all work; the provider client normalizes their different message formats and tool call conventions into a single interface. A separate fast model can be configured for the lighter planner and validator phases, keeping costs down while using a larger model only for final synthesis."},{label:"Scheduling and dispatch",body:"Dispatches are cron-scheduled via APScheduler running inside the server process. One agent can have multiple dispatches - different prompts, different schedules. Schedules persist across restarts via SQLite."},{label:"Web UI",body:"Dispatch is a single-page app served from the Python process. The Digest tab compiles the most recent output from every active agent into one briefing. The Active tab shows running dispatches with next-run times. Library tab is where agents are created, edited, and exported as YAML."},{label:"Deploy",body:"Dispatch ships with a systemd service file. Point it at the working directory, enable it, and the server runs as a background process with automatic restart on failure."},{label:"What I learned",body:["You gotta pay to play. When I first set out to build Dispatch my goal was for it to be completely self-contained - scraper, search, and inference all running locally. I ran into some hardware constraints based on the machine I was planning to deploy it on and pivoted to cloud APIs. What I got from the change was improved accuracy and speed while still remaining very low cost. The only cost is from the LLM calls, all searches are 100% free.","The harder decision was deciding between indexing and direct fetching. The web is made up of HTML/markdown and JavaScript, and parsing that can be difficult. Some websites I wanted to fetch were wrappers around an iFrame or other JS and I was unable to get the content in an LLM-parseable format. So the two options I have are to use the already-indexed search results, or use Jina to attempt to parse. Both are in use throughout the app and work well."]},{label:"Where I'd take it",body:["I'd love to have my digests delivered as notifications straight to my phone. That would make the idea of 'information comes to me' come full circle. I've explored using the ntfy iOS app but would prefer something more universal, possibly email.","I'd like agents to be able to call back to previous dispatch runs and refine their searches over time - effectively learning on their own what's most relevant or noisy."]}]}},{id:"packet",title:"Packet Pressure",tag:"Card game",desc:"An original card game that turns internet routing into social play. Players build shared paths through a network, hold endpoints, and negotiate when routes collapse. The logic of how packets actually move, made tangible.",meta:["Tabletop","Original"],href:"https://packetpressure.com"}],d=[];document.documentElement.classList.add("js");function T(e,t,n,o,a){let r="";for(let l=0;l<=64;l++){const u=l/64*Math.PI*2,S=Math.sin(u*3+a)*.5+Math.sin(u*5+a*1.7)*.3+Math.sin(u*2+a*.4)*.6,w=n*(1+S*o);r+=(l===0?"M":"L")+(e+Math.cos(u)*w).toFixed(1)+" "+(t+Math.sin(u)*w*.92).toFixed(1)+" "}return r+"Z"}function L(){const e=[];for(let t=0;t<6;t++){const n=(.85-t*.07).toFixed(2);e.push(`<path d="${T(0,0,8+t*11,.07+t*.01,t*2.1+1)}" fill="none" stroke="#b9682f" stroke-width="1.4" style="opacity:${n}" />`)}return`<svg viewBox="-66 -46 132 92" xmlns="http://www.w3.org/2000/svg" style="background:oklch(0.965 0.02 75)">
    ${e.join(`
    `)}
    <circle r="3.2" fill="#b9682f" />
  </svg>`}function E(){const e=["○","△","→","⊕","↑","⊘","□","↗"],t=["#5566c9","#4a90b8","#6aa86a","#9a8fc0","#c98a4a"],n=[];let o=0;for(let a=0;a<7;a++)for(let s=0;s<10;s++){const r=t[(a+s*2)%t.length],l=e[(a*3+s*5+o)%e.length];n.push(`<text x="${9+s*13.5}" y="${16+a*12}" font-size="9" font-family="monospace" fill="${r}" style="opacity:0.78">${l}</text>`),o++}return`<svg viewBox="0 0 132 92" xmlns="http://www.w3.org/2000/svg" style="background:#15171f">
    ${n.join(`
    `)}
  </svg>`}function P(){return`<svg viewBox="0 0 132 92" xmlns="http://www.w3.org/2000/svg" style="background:var(--bg-sunk)">
    <defs>
    ...
    <rect width="132" height="92" fill="url(#ph-hatch)" />
  </svg>`}const j={emanant:L(),dispatch:'<img src="/dispatch.png" alt="Dispatch — agent list view" />',packet:E()},f=A.map(e=>({...e,thumb:j[e.id]??P()})),M=[{label:"GitHub",href:"https://github.com/josterga"},{label:"LinkedIn",href:"https://www.linkedin.com/in/jamesostergaard/"},{label:"parcl0 — art & design",href:"https://parcl0.com"},{label:"Contact",href:"mailto:mail@jamesostergaard.com"}];function p(e){return String(e).padStart(2,"0")}function O(){return d.length>0&&location.hash==="#writing"?"writing":"projects"}function W(e,t){const n=e.meta.map(r=>`<span>${r}</span>`).join(""),o=!!e.caseStudy,a=o?`<span class="meta-actions"><span class="go">Case Study <span aria-hidden="true">→</span></span><a class="meta-ext-link" href="${e.href}" target="_blank" rel="noopener">View <span aria-hidden="true">↗</span></a></span>`:'<span class="meta-actions"><span class="go">View <span aria-hidden="true">↗</span></span></span>',s=`
      ...
      <div class="thumb">${e.thumb}</div>`;return o?`<div class="row" role="button" tabindex="0" data-project-id="${e.id}" data-reveal>${s}</div>`:`<a class="row" href="${e.href}" target="_blank" rel="noopener" data-reveal>${s}</a>`}function C(e,t){return`
    ...
    </div>`}function D(e){return`<a href="${e.href}"${e.href.startsWith("http")?' target="_blank" rel="noopener"':""}>${e.label} <span class="arr" aria-hidden="true">↗</span></a>`}function R(e){const t=e.caseStudy,n=e.meta.map(a=>`<span class="ov-dot" aria-hidden="true">·</span><span class="ov-meta-sm">${a}</span>`).join(""),o=t.sections.map((a,s)=>`
    ...
    </article>`}function q(e){var o;const t=e.read?`<span class="ov-dot" aria-hidden="true">·</span><span class="ov-meta-sm">${e.read} read</span>`:"",n=(o=e.body)!=null&&o.length?`<hr class="ov-rule" /><div class="ov-prose">${e.body.map(a=>`<p>${a}</p>`).join("")}</div>`:"";return`
    <article>
      <header class="ov-header">
        <div class="ov-eyebrow">
          <span class="tag">${e.kind}</span>
      ...
      ${n}
    </article>`}const g=O(),y=new Date().getFullYear(),B=`
  ...
  <div class="shell">
    <section class="hero">
      <p class="kicker" data-reveal>Portfolio · ${y}</p>
    ...
    <section id="work">
      ...
      <div class="tabs" role="tablist" data-reveal>
        <button class="tab" role="tab" data-tab="projects" aria-selected="${g==="projects"}">
          Projects <span class="ct">${p(f.length)}</span>
        </button>
        <button class="tab" role="tab" data-tab="writing" aria-selected="${g==="writing"}">
          Writing <span class="ct">${p(d.length)}</span>
        </button>
      </div>`:`
      <div class="sec-head" data-reveal><span>Projects</span><span class="count">${p(f.length)}</span></div>`}
      <div class="index" id="projects-index"${g!=="projects"?" hidden":""}>
        ${f.map((e,t)=>W(e,t+1)).join("")}
      </div>
      ${d.length>0?`
      <div class="index" id="writing-index"${g!=="writing"?" hidden":""}>
        ${d.map((e,t)=>C(e,t)).join("")}
      </div>`:""}
    </section>
    ...
    <footer class="foot">
      ...
      <div class="colophon" data-reveal>
        ...
        &copy; ${y} James Ostergaard
      </div>
    </footer>
  </div>
`,G=document.getElementById("root");G.innerHTML=B;const i=document.createElement("div");i.id="overlay";i.className="overlay";i.setAttribute("role","dialog");i.setAttribute("aria-modal","true");i.hidden=!0;document.body.appendChild(i);const z=`
  <div class="ov-bar">
  ...
  </div>`;let m=!1;function k(e){var t,n;i.innerHTML=z+`<div class="ov-scroll"><div class="ov-col">${e}</div></div>`,i.classList.remove("overlay-out"),i.hidden=!1,document.body.style.overflow="hidden",(t=document.getElementById("ov-back-btn"))==null||t.addEventListener("click",b),(n=document.getElementById("ov-close-btn"))==null||n.addEventListener("click",b)}function b(){m||(m=!0,i.classList.add("overlay-out"),setTimeout(()=>{i.hidden=!0,i.classList.remove("overlay-out"),document.body.style.overflow="",m=!1},340))}window.addEventListener("keydown",e=>{e.key==="Escape"&&!i.hidden&&b()});function F(e){e==="writing"?history.replaceState(null,"","#writing"):history.replaceState(null,"",location.pathname),document.querySelectorAll(".tab").forEach(o=>{o.setAttribute("aria-selected",String(o.dataset.tab===e))});const t=document.getElementById("projects-index"),n=document.getElementById("writing-index");t&&(t.hidden=e!=="projects"),n&&(n.hidden=e!=="writing")}document.querySelectorAll(".tab").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.tab;t&&F(t)})});function I(e){if(e.closest("a"))return;const t=e.closest("[data-project-id]");if(!t)return;const n=f.find(o=>o.id===t.dataset.projectId);n!=null&&n.caseStudy&&k(R(n))}const c=document.getElementById("projects-index");c==null||c.addEventListener("click",e=>I(e.target));c==null||c.addEventListener("keydown",e=>{e.key!=="Enter"&&e.key!==" "||e.target.closest("a")||(e.preventDefault(),I(e.target))});function $(e){const t=e.closest("[data-writing-idx]");if(!t)return;const n=parseInt(t.dataset.writingIdx,10),o=d[n];o&&k(q(o))}const h=document.getElementById("writing-index");h==null||h.addEventListener("click",e=>$(e.target));h==null||h.addEventListener("keydown",e=>{e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),$(e.target))});const v={paper:"#f9f8f5",dark:"#1b1712"};function J(){return document.documentElement.getAttribute("data-theme")||"paper"}function x(e,t){document.documentElement.setAttribute("data-theme",e);const n=document.getElementById("theme-color");if(n&&n.setAttribute("content",v[e]??v.paper),t)try{localStorage.setItem("jo-theme",e)}catch{}}document.querySelector(".theme-toggle").addEventListener("click",()=>{x(J()==="dark"?"paper":"dark",!0)});try{matchMedia("(prefers-color-scheme: dark)").addEventListener("change",e=>{let t=null;try{t=localStorage.getItem("jo-theme")}catch{}t||x(e.matches?"dark":"paper",!1)})}catch{}
